...
export class UserInfrastructure implements UserRepository {
    ...
    async getById(userId: string): Promise<User | null> {
        const repository = DatabaseBootstrap.datasource.getRepository(UserEntity)
        const userReturned = await repository.findOne({ where: { userId, deletedAt: IsNull() } })
    ...
    async getAll(): Promise<User[]> {
        const repository = DatabaseBootstrap.datasource.getRepository(UserEntity)
        const usersReturned = await repository.find({ where: { deletedAt: IsNull() } })
    ...
    async getByPage(page: number, limit: number): Promise<ResponsePage<User>> {
        ...
        const [usersReturned, total] = await repository.findAndCount({ where: { deletedAt: IsNull() }, skip: (page - 1) * limit, take: limit })
    ...
    async existsEmail(email: string): Promise<boolean> {
        ...
        const userMatched = await repository.findOne({ where: { email, deletedAt: IsNull() } })
