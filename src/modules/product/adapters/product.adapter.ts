...
export class ProductAdapter implements ProductPort {
    async create(product: Product): Promise<Product> {
        const repository = DatabaseBootstrap.datasource.getRepository(ProductEntity)
    ...
    async get(productId: number): Promise<Product | null> {
        ...
        const productReturned = await repository.findOne({ where: { productId, deletedAt: IsNull() } })
    ...
    async getAll(): Promise<Product[]> {
        ...
        const productsReturned = await repository.find({ where: { deletedAt: IsNull() } })
    ...
    async getByPage(page: number, size: number): Promise<ResponsePage<Product>> {
        ...
        const [productsReturned, total] = await repository.findAndCount({ where: { deletedAt: IsNull() }, skip: (page - 1) * size, take: size })
        return { results: productsReturned.map(product => new Product(product)), total }
    }

    async searchByName(name: string): Promise<Product | null> {
        const repository = DatabaseBootstrap.datasource.getRepository(ProductEntity)

        const productReturned = await repository.findOne({ where: { name: Like(`%${name}%`), deletedAt: IsNull() } })
